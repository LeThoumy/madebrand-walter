    ...
    if ( this.length > 1 ) {
      ...
      this.SCROLL_EVENT = debounce(()=>{
        ...
        if ( ! this._sliderBlockScroll ) {
          ...
          const scrollArray = Array.from(scrollItems, elm => Math.abs(elm[1].offsetLeft-this.scrollLeft));
		...
		this.querySelectorAll('video').forEach(elm=>{
			isAutoplaySupported(canAutoPlay=>{
				if(canAutoPlay) {
					...
					const fallback = elm.closest('.slide-image');
					elm.parentNode.remove();
					window.preloadImages(fallback);
				}
			})
		});
